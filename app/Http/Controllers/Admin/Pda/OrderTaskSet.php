...
class OrderTaskSet extends Controller
{
    ...
    public function index(Request $request){
        ...
        $data = self::$BoundTask->where('tesk_status', 0)->whereIn('two_logisticsOrderCode', $request->taskID)->select('id', 'two_logisticsOrderCode','mailNo')->get()->toArray();
        ...
        try {
            self::$BoundTask->where('tesk_status', 0)->whereIn('two_logisticsOrderCode', $request->taskID)->update(['tesk_status' => 1, 'admin_name' =>$adminInfo->user_name]);
