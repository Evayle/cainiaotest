...
class OrderTaskSet extends Controller
{
    ...
    public function index(Request $request){
        ...
        $data = self::$BoundTask->where('tesk_status', 0)->where('two_logisticsOrderCode', $request->taskID)->select('id', 'two_logisticsOrderCode','logisticsOrderCode')->get()->toArray();
        ...
        foreach ($data as $key => $val){
            ...
            $dataLog[$key]['order']     = $val['logisticsOrderCode'];
